...
var (
	...
	runAs      = os.Getenv("RUN_AS")
)

func main() {
	if runAs == "library" {
		runIntegrationTestAsLibrary()
	} else {
		runIntegrationTest()
	}
}
...
var cidrLibraryTestcases = map[string]TestCase{
...
func runIntegrationTestAsLibrary() {
	for name, testCase := range cidrLibraryTestcases {
		if customTest != "" && !strings.Contains(name, customTest) {
			continue // only run tests user asked
		}
		fmt.Printf("Running test cases for \"%s\"\n", aurora.Cyan(name))
